		...
		.done( function(){
			...
			for (var i = 0; i < data.responseJSON.data.length; i++) {
                if(data.responseJSON.data[i][15] == "Manhattan" && data.responseJSON.data[i][9] != "CONFIDENTIAL"){
                    ...
                    var distance = 1000*getDistanceFromLatLonInKm(uLat,uLng,data.responseJSON.data[i][23],data.responseJSON.data[i][24]);
                    ...
                    //newMarker.addListener('click', function() {
                        //document.getElementById("descripcion").innerHTML = newHTML;
                    //});
...
function getMaxTemp(){
    ...
    headers:{'token':'VmuFkPVjDPVPYLTTfeNTfhoYJmNiuCEQ'}})
...
function avrgTemp(){
    for(var i = 0; 0 < minTempArr.length;i++){
        avrgTempArr.push((minTempArr[i][0] + maxTempArr[i][0])/2);
    }
    console.log(avrgTempArr);
}
...
$(document).ready(function(){
	...
	$("#byDistance").on("click", markHousesbyDistance);
	//$("#askData").on("click", updateInfo);
	$("#generic").on("click", spitTemp);
});