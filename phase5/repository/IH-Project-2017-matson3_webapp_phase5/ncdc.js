...
class Seasonal {
	constructor(spring = 0, summer = 0, autumn = 0, winter = 0) {
...
function NCDCgetAnnualPrecipitationDays(id) {
	return new Promise(function(resolve, reject){
		...
		NCDCdataGet("NORMAL_ANN", "SON-PRCP-AVGNDS-GE100HI"/*"ANN-SNWD-AVGNDS-GE001WI"*/, startDate, endDate, id, errorWrapper)
		.done( (result) => {
			returnTemp.a[3] = Math.round(extractBestAverage(result));
			if(returnTemp.a[3] === null) { console.log(result); reject("Could not find valid winter temperature"); }
			if(returnTemp.set) resolve(returnTemp);
		});
